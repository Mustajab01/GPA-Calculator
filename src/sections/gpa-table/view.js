	...
	return (
		...
		>
						...
						{gradeScores.map((grade, index) => (
							...
							>
								...
								<td className="p-4 text-center">
									<span
										className={`inline-flex items-center gap-1 font-bold ${getGradeColor(
											grade.gradePoint
										)}`}
									>
										<Award size={16} />
			...
			<div className="flex justify-center items-center gap-3">
				...
				<div className="mt-8 flex justify-center">
					<Link
						href="https://www.uok.edu.pk/faculties/computerscience/bs.php"
						target="_blank"
						rel="noopener noreferrer"
						className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-6 py-3 rounded-lg hover:opacity-90 transition duration-300 font-medium"
					>
						<School size={20} />
						UBIT Official Website
					</Link>
				</div>
			</div>
		</div>
